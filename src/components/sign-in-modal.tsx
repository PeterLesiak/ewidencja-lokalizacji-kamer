...
function SignInForm({ close, ...props }: ComponentProps<'form'> & { close: () => void }) {
  ...
  async function onSubmit(data: z.infer<typeof signInSchema>) {
    ...
    for (const [field, error] of Object.entries(state)) {
      form.setError(field as keyof typeof state, { message: error.errors[0] });
      form.setFocus(field as keyof typeof state);
    }
  }
